...
from taxlover.utils import get_income_years, create_or_get_assets_obj


class AssetsDTO:

    def __init__(self, tax_payer_id, has_form_error, previous_year=None):
        assets = create_or_get_assets_obj(tax_payer_id, True, previous_year)
        self.cash_assets = get_cash_assets_by_payer(tax_payer_id, previous_year)
        self.previous_year_net_wealth = get_net_wealth_by_payer(tax_payer_id, previous_year)
        ...
        self.agricultural_properties = get_agricultural_property_by_payer(tax_payer_id, previous_year)
        self.total_agricultural_properties_value = get_total_agricultural_property_value(self.agricultural_properties)
        self.investments = get_investments_by_payer(tax_payer_id, previous_year)
        ...
        self.total_other_investment_investment_value = get_total_investment_value_by_type(self.investments,
                                                                                           'Other Investment')
        self.motor_vehicles = get_motor_vehicle_by_payer(tax_payer_id, previous_year)
        ...
        self.furnitures = get_furniture_by_payer(tax_payer_id, previous_year)
        self.jewelleries = get_jewellery_by_payer(tax_payer_id, previous_year)
        self.total_jewellery_value = get_total_jewellery_value(self.jewelleries)
        self.electronic_equipments = get_electronic_equipment_by_payer(tax_payer_id, previous_year)
        ...
        self.other_assets = get_other_assets_by_payer(tax_payer_id, previous_year)
        self.other_assets_receipts = get_other_assets_receipt_by_payer(tax_payer_id, previous_year)
